...
//cos.jar에서 제공하고있는 FileRenamePolicy 인터페이스를 구현해야한다
public class MyFileRenamePolicy implements FileRenamePolicy{
	
	
	//미완성된 rename메소드를 오버라이딩하여 구현하기
	//기존의 파일명을 받아서 파일명 수정작업을 거친 후에 수정된 파일을 반환해주는 메소드를 구현할것
	...
	public File rename(File originFile) {
		...
		String currentTime = new SimpleDateFormat("yyyyMMddHHmmss").format(new Date()); //java.util패키지
		
		//2. 5자리 랜덤값 -> int ranNum;
		int ranNum = (int)(Math.random()*90000)+10000;
		
		//3. 원본파일의 확장자 추출 String ext
		//파일명 중간에 . 이있을 가능성도 있으니 맨뒤에서부터 가장 처음 오는 .을 찾아주면된다 
