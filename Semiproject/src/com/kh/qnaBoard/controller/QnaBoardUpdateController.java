...
public class QnaBoardUpdateController extends HttpServlet {
     ...
     */
    public QnaBoardUpdateController() {
        super();
...
//		전달된 파일명을 수정작업 후 서버에 업로드 
			...
			int QnaBoardNo = Integer.parseInt(multiRequest.getParameter("boardNo"));
			String QnaBoardTitle = multiRequest.getParameter("boardTitle");
			String QnaBoardContent = multiRequest.getParameter("boardContent");
			String QnaCategory = multiRequest.getParameter("category");

			QnaBoard b = new QnaBoard();
			b.setQnaBoardNo(QnaBoardNo);
			...
			int result = new QnaBoardService().updateQnaBoard(b,at);
			
			if(result>0) {
				//성공시엔 게시글 상세 페이지로 재요청
				request.getSession().setAttribute("alertMsg","게시글 수정 성공");
				response.sendRedirect(request.getContextPath()+"/detail.qbo?qbno="+QnaBoardNo);
			}else {
